...
static int usb_write_serial(char *cmd, int param) {
    int ret, actual_size, size;

    printk(KERN_INFO "SmartLamp: Enviando comando: %s %d\n", cmd, param);

    size = snprintf(usb_out_buffer, usb_max_size, "%s %d\n", cmd, param); // montando

    ret = usb_bulk_msg(smartlamp_device, usb_sndbulkpipe(smartlamp_device, usb_out),usb_out_buffer, size, &actual_size, 1000);
    
    if (ret){
        printk(KERN_ERR "SmartLamp: Erro %d\n", ret);
        return ret;
    }
